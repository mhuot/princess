...
The `repository-meta` capability covers the project's distribution surface: the README and supporting docs (CHANGELOG, CONTRIBUTING, LICENSE, NOTICE), GitHub PR and issue templates, the CI workflows that run tests, lint, and OpenSpec validation, the OpenSpec configuration that carries project context and workflow rules, and the atomic-commit / docs-in-sync policy that keeps user-visible documentation aligned with implementation.
...
The README SHALL mention the **mobile URL**: a friend on a phone visits `<host>:8000/m` (or `<host>:8000/m/<code>` to join a specific room) for a touch-friendly UI; the desktop UI at `/` is unchanged.
...
#### Scenario: Mobile URL mentioned

- **WHEN** the README is rendered
- **THEN** it mentions the mobile route `/m` (and `/m/<code>` for joining a specific room) and clarifies that the desktop UI at `/` is unchanged
