...
The `deployment` capability covers how Princess ships and runs in production: the top-level `Dockerfile` that builds a minimal runnable server image, the `docker-compose.yml` that wires the `princess` container into the shared `nginx-proxy-network`, the nginx-director registration that fronts the service with HTTP and WebSocket support, and the GitHub Actions `deploy.yml` workflow that auto-deploys `main` onto a self-hosted runner with a post-deploy smoke check. It also locks in the project's naming policy across every operator-visible deployment surface.
